...
```
最后,你可以运行`sudo redis-sentinel /etc/redis/sentinel.conf`
...
##关于redis监控
- redis的系统报告可以使用`redis-cli info`,`redis-cli -h localhost -p 6380 monitor `
- 使用redis-benchmark工具,作为性能检测. `redis-benchmark -h localhost -p 6379 -c 100 -n 100000 `
- 一些web方式的监控工具:
  * redis-monitoa: https://github.com/PrettySimple/redis-monitorr
